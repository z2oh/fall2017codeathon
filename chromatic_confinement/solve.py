...
def build_kd_tree(vecs, dim, d):
	...
	tree.val = vals[mid]
	tree.dim = dim
	tree.left = build_kd_tree(vals[:mid-1], (dim+1)%d, d)
	if(tree.left != None):
		tree.left.parent = tree
	tree.right = build_kd_tree(vals[mid:], (dim+1)%d, d)
	if(tree.right != None):
...
def find_nearest_neighbor(root, color, top):
	...
	while(root.left != None and root.right != None):
		if(color[root.dim] < root.val[root.dim]):
			root = root.left
		elif(color[root.dim] >= root.val[root.dim]):
			root = root.right
